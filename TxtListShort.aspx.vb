	...
	Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
		...
		If sWhereSeznam <> "" Then
			...
			While DR.Read()
				...
				If Sekce.Alias = "Citaty" Then
					Txt = "[<a href=""/Citaty/autorinfo-" & DR("Autor") & ".aspx"">" & Server.HtmlEncode(DR("AutorJmeno")) & "</a>]" & vbCrLf & Txt
				End If
				...
				sHtml &= "<p class=""title""><span style=""float: left;"">Poslal(a) " & Server.HtmlEncode(Left(DR("Poslal"), 23)) & " | " & CType(DR("Datum"), Date).ToString("d.M.yyyy") & " | <a href=""/" & Sekce.Alias & "/" & iID & "-hodn.aspx"" class=""pamatovatklik"">hodnocení</a> " & DR("Hodnoceni") & "%</span>"
				sHtml &= "	<span style=""float: right;"">(<a href=""/SendEmail.aspx?sekce=" & Sekce.Alias & "&amp;id=" & iID & """>EMAIL</a> " & DR("Odeslano") & " x)</span>&nbsp;"
				sHtml &= "</p>"
				sHtml &= "</div>"
				f += 1
			End While
